---
title: Modal Routes
description: Modal Routes can navigate modal routes with real-time URL changes for seamless browsing.
---
...
First, update your [`app.vue`](https://nuxt.com/docs/guide/directory-structure/app) file with [`<PlusModalApp />`](/components/plus-modal-app).

`PlusModalApp` renders both `<NuxtLayout>` and `<NuxtPage>`. When a modal opens, it keeps the background route and that route's layout rendered underneath the modal.
...
If you need a custom app tree, use its scoped slot. The slot provides the modal-aware `route` and `layout`:
...
::note
Existing applications can continue using [`<PlusModalNuxtPage />`](/components/plus-modal-nuxt-page). Its default rendering remains page-only for backward compatibility, so it does not add `<NuxtLayout>` unless you do so in its scoped slot.
::
...
### Displaying a Custom Modal

If you display your own modal UI instead of `<PlusModalPage>` (for example an inline `<Teleport>` or a dialog component from a UI library), gate it with `$modalRouter.isOpen` instead of checking the current route.

While a navigation is pending (for example when the target page has an async `setup` or is fetching data), the global route and the parallel routers are already updated to the target route, but the previous page is still displayed. A modal gated only by the route would therefore pop up over the previous page even when the target history entry should render as a plain full page — for example when revisiting a refreshed modal route with the browser back / forward buttons. `isOpen` only becomes `true` when the current history entry actually carries a modal background view.
...
- [Example - Gallery](https://github.com/serkodev/nuxt-pages-plus/tree/main/examples/modal-gallery)
